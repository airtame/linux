...
/* status codes for reading one byte of EDID data over i2c*/
#define EDID_I2C_READ_SUCCESS 1
#define EDID_I2C_READ_FAILED 0
...
static u8 hdmi_edid_i2c_read(struct mxc_hdmi *hdmi,unsigned char *edidata,
					u8 addr, u8 blockno)
{
	...
	if (result) {
		data = hdmi_readb(HDMI_I2CM_DATAI);
               *edidata = data;
		hdmi_writeb(0xFF, HDMI_IH_I2CM_STAT0);
		return EDID_I2C_READ_SUCCESS;
	}
	hdmi_writeb(0xFF, HDMI_IH_I2CM_STAT0);
	return EDID_I2C_READ_FAILED;
}
			...
			struct mxc_edid_cfg *cfg, struct fb_info *fbi)
{
	...
	if ((!((hdmi_edid_i2c_read(hdmi,ediddata,0,0)) && ((*ediddata) == 0x00))) ||
	...
	for (i = 0; i < 128; i++) {
		u8 result = hdmi_edid_i2c_read(hdmi,ediddata, i, 0);
		if (!result) {
	...
	if (extblknum) {
		...
		for (i = 0; i < 128; i++) {
			u8 result = hdmi_edid_i2c_read(hdmi,ediddata, i, 1);
			if (!result) {
	...
	if (extblknum > 1) {
		for (j = 2; j <= extblknum; j++) {
			for (i = 0; i < 128; i++){
				u8 result = hdmi_edid_i2c_read(hdmi,&tmpedid[i], i, j);
				if (!result) {
