...
static void mxc_hdmi_edid_rebuild_modelist(struct mxc_hdmi *hdmi)
{
	...
	for (i = 0; i < hdmi->fbi->monspecs.modedb_len; i++) {
		...
		if (!(mode->vmode & FB_VMODE_INTERLACED)) {
            ...
            //fb_validate_mode detects wrongly some video modes that are valid
            // i don't know if their is an issue in the function or some paramethers for the
            // max display values are not computed correctly. Therefore I comment the call to
            //this function. This means that all the video modes that are advertised by the
            //display are allowed.
