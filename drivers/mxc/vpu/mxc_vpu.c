...
#define POOL_MEM_SIZE (PAGE_SIZE * 16<<10) //64MB
...
static struct vpu_mem_desc pool_mem = { 0 };
...
{
	int rc = 0;
	pool_mem.size = POOL_MEM_SIZE;
	rc = vpu_alloc_dma_buffer(&pool_mem);
	if (rc == -1) {
		return rc;
	}
...
static int vpu_dealloc_dma_buffer_pool(void)
{
	...
	if (pool_mem.cpu_addr != 0) {
		vpu_free_dma_buffer(&pool_mem);
		dev_dbg(vpu_dev, "[FREE] freed paddr=0x%08X\n", pool_mem.phy_addr);
	}
	return 0;
}
 ...
 */
static bool vpu_acquire_dma_buffer_from_pool(void)
{
	if (atomic_read(&pool_users) == 0) {
		atomic_inc(&pool_users);
		return true;
	}
	return false;
}
...
{
	if (atomic_read(&pool_users) == 1) {
		atomic_dec(&pool_users);
		return true;
	}
	return false;
}
		     ...
		     u_long arg)
{
	...
	case VPU_IOC_ACQUIRE_PHYMEM_POOL:
		{
			/*
			 * If the function returns false that means that the preallocated memory resource
			 * busy.
			 */
			if (!vpu_acquire_dma_buffer_from_pool()) {
				return -EBUSY;
			}
			ret = copy_to_user((void __user *)arg, &(pool_mem),
	...
	case VPU_IOC_RELEASE_PHYMEM_POOL:
		{
			/*
			 * If we can't release that memory to the pool, return the error code.
			 */
...
static int vpu_release(struct inode *inode, struct file *filp)
{
	...
	if (open_count > 0 && !(--open_count)) {
		...
		// TODO: Release allocated memory per file descriptor
		vpu_release_dma_buffer_to_pool();
		vpu_free_buffers();
