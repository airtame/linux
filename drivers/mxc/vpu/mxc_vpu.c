	...
	case VPU_IOC_PHYMEM_ALLOC:
		{
			...
			mutex_lock(&vpu_data.lock);
			/*******/
			dev_dbg(vpu_dev, "\t[ALLOC_TYPE]\t[STATUS]\t[CPU_ADDR]\t[PHY_ADDR]\t[SIZE]\n");
			list_for_each_entry_safe(rec_dbg, n, &head, list) {
				dev_dbg(vpu_dev, "\t%s\t%s\t0x%x\t0x%x\t%u\n", rec_dbg->is_preallocated ? "STATIC" : "DYNAMIC",
						rec_dbg->is_busy ? "RESERVED" : "AVAILABLE",
	...
	case VPU_IOC_PHYMEM_FREE:
		{
			...
			list_for_each_entry_safe(rec, n, &head, list) {
				if (rec->mem.cpu_addr == vpu_mem.cpu_addr && !release_to_pool) {
					dev_dbg(vpu_dev, "X %s\t%s\t0x%x\t0x%x\t%u\n",
							rec->is_preallocated ? "STATIC" : "DYNAMIC",
							rec->is_busy ? "RESERVED" : "AVAILABLE",
							rec->mem.cpu_addr, rec->mem.phy_addr, rec->mem.size);
					/* delete from list */
